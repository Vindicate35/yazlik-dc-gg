...
v5.7.0 - Dinamik Keskin Nişancı Filtresi

•	Bireysel profillerde sağ taraftaki rol kartlarına (Örn: Nişancı, Üst) tıklandığında sol taraftaki maç listesinin anında o role göre süzülmesi sağlandı (Interactive Filtering).
•	Bireysel maç filtreleme barına prestij rozetleri (Penta, Çalıntı, İlk Kan) için dinamik butonlar eklendi.

v5.6.0 - Dinamik Kimlik Çözümleme (Smurf Entegrasyonu)

•	Yan hesapların (Smurf) ve eski isimlerin ana kimlikle birleştirildiği (Data Consolidation) "Dynamic ID Resolution" altyapısı kuruldu.
•	Liderlik tablolarında ve şampiyon sekmelerinde sistemin otomatik olarak güncel ana hesabı göstermesi, geçmiş maç kartlarında ise o günkü nostaljik ismin (Alias) mühürlü kalması sağlandı.

v5.5.1 - Kapsamlı Profil Özeti

•	Bireysel profil özet panosu 4 kutudan 5 kutuya çıkarılarak, "Rol & Donanım" istatistikleri ile yeni eklenen "Vurgunlar & Başarılar" verileri kusursuz bir yatay simetriyle birleştirildi.
•	"Kusursuz Maç" ve "Sabotaj" istatistikleri aynı satırda birleştirilerek dikey alandan tasarruf edildi.

v5.5.0 - Prestij ve Kozmetik Avcısı

•	Arka planda toplanan Pentakill, Quadrakill, Çalınan Objektif ve İlk Kan (First Blood) verileri arayüze entegre edildi.
•	Bireysel profil özetine "Vurgunlar & Başarılar" paneli ve maç kartlarına parlayan özel animasyonlu rozetler eklendi.
•	Tarih etiketlerine yıl verisi (Örn: 2026) eklenerek geçmiş sezonlardaki kazıların kronolojik olarak anlaşılması sağlandı.

v5.4.4 - Sinerji Bağlantı Yaması

•	Alt Koridor listesindeki ikililere (Nişancı + Destek) tıklandığında maçların listelenmesini engelleyen İsim Eşleştirme (String Matching) hatası giderildi. Alt koridor tıklamaları için özel isim çözümleyici yazıldı.

v5.4.3 - Kusursuz Rüya Takımı

•	"Tüm Zamanların Rüya Takımı" ve "Haftanın 5'lisi" panolarına, arka plandaki algoritmada zaten var olan CS/min (Minyon/Dakika) istatistikleri görsel olarak çivilendi. Destek rolü, adaleti sağlamak için bu görselden muaf tutuldu.

v5.4.2 - Kompakt Kanban Dizilimi

•	Şampiyon uzmanları sekmesinde ekran daraldığında ortaya çıkan tasarım (flex-wrap) hatası giderildi.
•	İstatistiklerin ismin altına kaymasını engelleyen ve boşlukları yok eden jilet gibi yan yana (Kanban stili) dizilim getirildi.
